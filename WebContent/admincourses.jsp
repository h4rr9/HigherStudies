...
<body>
<%@page import="entities.Course, java.util.ArrayList, java.util.Arrays"%>
<%ArrayList<Course> courses=(ArrayList<Course>) request.getAttribute("courses");%>

<div class = "topnav">
      <a href = "<%=response.encodeURL(request.getContextPath() + "/AdminController?action=users")%>">Users</a>
      <a class = "active" href = "<%=response.encodeURL(request.getContextPath() + "/AdminController?action=courses")%>">Courses</a>
      ...
      <p style="float: right; color: white;"><%=session.getAttribute("name")%></p> 
 </div>
...
<div class="container">
        ...
        <table class="table table-striped table-bordered" style="margin-top: 5%;">
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-size: 15px"><%=course.getName()%></td>
                    <td style="font-size: 15px"><%=course.getAbout()%></td>
                    <td style="font-size: 15px"><%=course.getImage()%><br></td>
                    <td style="font-size: 15px"><%=course.getProspects()%><br></td>
                    <td>
                    	<form onsubmit="return confirm('Are you Sure?');" action="<%=response.encodeURL(request.getContextPath() + "/CourseController?action=delete")%>" method="post">
                    ...
                    <td>
                    	<form method="get" action="<%=response.encodeURL(request.getContextPath() + "/CourseController")%>">
